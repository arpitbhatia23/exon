...
<ParamField path="add <plugin>" type="command">
  Adds a new plugin (e.g., `docker`, `prisma`, `mongoose`, `drizzle`) to an existing Exon project.
</ParamField>

<ParamField path="remove <plugin>" type="command">
  Removes an existing plugin and its associated files from the project.
</ParamField>
